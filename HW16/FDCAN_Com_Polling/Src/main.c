...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
// INA219 I2C Address and Register Map
...
// PI Controller Gains (You will need to tune these in Python!)
float kp = 1.5f; 
float ki = 0.05f; 
float desired_current = 150.0f; // Target current in milliamps

// Data Buffers (400 samples = 400 milliseconds)
int log_index[400];
float log_desired[400];
float log_actual[400];
/* USER CODE END PV */
...
int main(void)
{
  ...
  // Start the 20kHz PWM on both channels (Set to 2400 = OFF/Brake)
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400);
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 2400);
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_2);

  // Start the 1kHz interrupt timer
  HAL_TIM_Base_Start_IT(&htim2);

  printf("Ready! Send 'a' to trigger the step response.\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    uint8_t rx_byte = 0;
      ...
      // 1. Wait for serial input from Python
      if (HAL_UART_Receive(&huart2, &rx_byte, 1, 10) == HAL_OK) {
          if (rx_byte == 'a') {
              
              // 2. Trigger the control loop
              state = 1;
              
              // 3. Block this loop until the interrupt finishes 400 cycles
              while (state == 1) {
                  // Do nothing, let the 1kHz interrupt do the work
              }
              
              // 4. Send the recorded data back to Python in CSV format
              for (int i = 0; i < 400; i++) {
                  // Printing floats can be tricky on STM32, so we cast to int for safe transmission
                  printf("%d, %d, %d\r\n", log_index[i], (int)log_desired[i], (int)log_actual[i]);
              }
              
              // Ensure desired current is reset for the next run
...
/* USER CODE BEGIN 4 */
// Retargets printf() to output via the Virtual COM Port (USB)
int _write(int file, char *ptr, int len) {
    HAL_UART_Transmit(&huart2, (uint8_t*)ptr, len, HAL_MAX_DELAY);
    return len;
}

// Polling wrapper for the ADC (Potentiometer)
uint32_t read_adc(void) {
    uint32_t raw;
    HAL_ADC_Start(&hadc1);                                 
...
// Read current as a float (Will not print correctly via printf without linker flags)
float read_ina219(void) {
    float ma = 0;
